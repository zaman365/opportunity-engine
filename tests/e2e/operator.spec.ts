...
async function runScan(page: Page, targetUrl: string): Promise<string> {
...
test.describe('operator journey', () => {
  ...
  test('queue, case, review and report', async ({ page }, testInfo) => {
    ...
    const caseLink = page.getByRole('link', { name: /Linked information page returns an error/ }).first();
    ...
    await page.screenshot({ path: `${SHOTS}/${testInfo.project.name}-case-evidence.png`, fullPage: true });
    ...
    await page.getByLabel('Reviewer note').fill('Both recorded checks returned 404 for the linked size guide.');
    // Still disabled: the limits have not been acknowledged.
    await expect(confirm).toBeDisabled();
    await page.getByRole('checkbox').check();
    await expect(confirm).toBeEnabled();
    await page.screenshot({ path: `${SHOTS}/${testInfo.project.name}-review-seam.png`, fullPage: true });
  ...
  test('a blocked scan explains itself and produces no finding', async ({ page }, testInfo) => {
    ...
    await expect(page.locator('.notice[data-tone="blocked"], .notice[data-tone="attention"]')).toBeVisible();
    await expect(page.locator('body')).not.toContainText('404');
    await page.screenshot({ path: `${SHOTS}/${testInfo.project.name}-scan-blocked.png`, fullPage: true });
  });
  ...
  test('a healthy page reports no supported defect rather than good health', async ({ page }) => {
    ...
    await expect(page.locator('.coverage')).toContainText('This is a sample, not a whole-store inspection');
  ...
  test('review conflict preserves the typed note', async ({ page }, testInfo) => {
    ...
    const target = (opportunities as { items: { id: string; finding_ids: string[] }[] }).items.find((item) =>
  ...
  test('an unavailable artifact states why, and the observation still stands', async ({ page }, testInfo) => {
    test.skip(testInfo.project.name !== 'desktop', 'One projection of this scenario is enough.');
    const scanId = await runScan(page, FIXTURE_PRODUCT);
    const scan = await (await page.request.get(`/api/v1/scans/${scanId}`)).json();
    ...
    const firstCell = (timeline as { evidence: { id: string; conditions: { session_id: string } }[] }).evidence.find(
      (item) => item.conditions.session_id.endsWith(':source_page:1'),
    )!;
    ...
    const opportunities = await (await page.request.get('/api/v1/opportunities?limit=100')).json();
    const target = (opportunities as { items: { id: string; finding_ids: string[] }[] }).items.find((item) =>
      item.finding_ids.includes((scan as { finding_ids: string[] }).finding_ids[0]!),
    )!;
    ...
    await expect(page.locator('.stage')).toContainText('The recorded observation below is unaffected');
...
test.describe('workspace surfaces', () => {
  test('settings shows cost limits and states what is absent', async ({ page }, testInfo) => {
    ...
    await page.screenshot({ path: `${SHOTS}/${testInfo.project.name}-settings.png`, fullPage: true });
  });
  ...
  test('the new-scan form refuses an unapproved host before submitting', async ({ page }, testInfo) => {
    ...
    await page.screenshot({ path: `${SHOTS}/${testInfo.project.name}-new-scan-blocked.png`, fullPage: true });
  });

  test('the composition studies render both alternatives on the same content', async ({ page }, testInfo) => {
...
test.describe('accessibility behaviour', () => {
  test('keyboard reaches the case and the artifact viewer returns focus', async ({ page }, testInfo) => {
    ...
    await page.getByRole('link', { name: /Linked information page returns an error/ }).first().click();
