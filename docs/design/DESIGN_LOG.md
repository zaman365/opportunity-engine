...
application against the kit's synthetic loopback fixture. None of them shows a real merchant,
and none of the observations in them is evidence about one.
...
[ADR-016](../adr/ADR-016-composition.md). B's shape is used for the *report* surface, where the
...
*1440×960, populated case, logo hidden.*
...
1. **The inspected-page row read "Not captured" on a complete scan.** The case was drawing the
   matrix from the opportunity payload, which carries only the evidence a *finding cites* — for
...
*Heuristic walkthrough: queue → case → evidence → decision → report, keyboard only, then at
1440, 1080, 768, 390 and 320px.*

Confirmed working, with browser assertions behind each:

- The confirm action is disabled until a note of at least five characters exists *and* the
...
*1440×960 and 390×844, alignment, truncation, numerals, focus.*
