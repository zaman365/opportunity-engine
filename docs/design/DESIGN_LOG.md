...
## Pass 5 · the eligible scope, below the decision

_1440×960 and 390×844, a confirmed MF-LINK-01 case._

The catalogue panel sits **below** the review seam and the report action, not beside them. That
ordering is the argument: a claim is settled before it is priced, and a reviewer who sees a
price next to an undecided finding is being invited to decide with the price in view.

What the panel shows, and why each part is there:

- **Price and effort band on one line with the SKU.** `290.00 EUR net · 1–3 h`. A range, not a
  point estimate, because nobody has delivered one of these for a paying customer yet.
- **Excludes, at the same weight as includes.** "guaranteed revenue uplift" is on screen
  beside the price. A promise and its limits travel together or the limits are decoration.
- **Prerequisites, named, when they are missing.** Not "unavailable" — the three specific
  things an owner has to record. A reviewer who cannot see why a scope is blocked will invent
  an explanation.
- **No price input anywhere.** There is no field, and the browser test asserts its absence
  rather than trusting that nobody adds one later.

**Found and fixed**

13. **An eligible-but-blocked scope was counted twice.** The matcher reports a scope with unmet
    prerequisites as both eligible and rejected, which is correct for the API — a caller needs
    the reason — but the panel rendered both, so the screen said "MF-LINK-REPAIR, here is its
    price" and "1 scope is unavailable" at once. The card already states the reason in full, so
    the disclosure now lists only scopes with no card. Caught by reading
    `desktop-scope-blocked.png`, not by a failing test; the test came after.

**Still open here:** the drafted card shows the snapshot's promise but not its full scope. Once
a draft can be sent to anybody, the reviewer has to be able to read exactly what the customer
will read, from this screen, without going back to the catalogue. Not needed while a draft is
internal; needed before the first one leaves the building.
...
| `desktop-scope-blocked.png`, `narrow-scope-blocked.png`       | Eligible scope, prerequisites unmet      |
| `desktop-scope-drafted.png`, `narrow-scope-drafted.png`       | Drafted scope and its withdrawal control |
...
- A case with several eligible scopes. The seeded catalogue offers one per venture, so the
  panel has never been seen with two cards competing for attention, and the ordering between
