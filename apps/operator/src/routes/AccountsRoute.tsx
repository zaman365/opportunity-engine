    ...
    <>
      <div className="pagehead">
        <div>
          <h1>Accounts</h1>
          <p>
            A business plus the exact hosts an owner has approved for inspection. An operator cannot
            widen this list.
          </p>
        </div>
      </div>
      ...
      {resource.status === 'ready' && resource.data.items.length === 0 ? (
        <EmptyState kind="workspace" title="No accounts yet">
          An owner records an account with its canonical domain, the hosts approved for inspection
          and where that approval came from.
        </EmptyState>
      ) : null}
      ...
      {session.role === 'owner' ? (
        ...
        >
          Creating an account and recording its scan authorization are owner API operations. A form
          for them is not built in this milestone.
        </p>
      ) : null}
    </>
  );
