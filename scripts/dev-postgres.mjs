...
/** 48 hex characters. Keys the verification hashes and the rate-limit keys; local only. */
...
function writeEnvFile(creds) {
  const body = [
    ...
    // On locally, because the local verification channel sends nothing: it hands the code
    // back to the caller and refuses to construct outside APP_ENV=local. Off everywhere else
    // until there is an owner-approved way to reach a member of the public.
    'PUBLIC_INTAKE_ENABLED=true',
    'INTAKE_VERIFICATION_CHANNEL=recorded_local_only',
    `INTAKE_SECRET=${randomSecret()}`,
